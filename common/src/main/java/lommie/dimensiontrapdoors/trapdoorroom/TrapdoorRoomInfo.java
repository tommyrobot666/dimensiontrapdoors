...
public record TrapdoorRoomInfo(BlockPos relativeSpawnPos, int x, int y, Identifier structure, Optional<Identifier> biome, int roomRegionId, int chunksSize) {
    public static final Codec<TrapdoorRoomInfo> CODEC = RecordCodecBuilder.create((instance) ->
            instance.group(
                    ...
                    Codec.INT.fieldOf("chunksSize").forGetter(TrapdoorRoomInfo::chunksSize)
    ...
    public static TrapdoorRoomInfo fromType(TrapdoorRoomType trapdoorRoomType, int x, int y, int roomRegionId) {
        return new TrapdoorRoomInfo(trapdoorRoomType.relativeSpawnPos(),x,y,trapdoorRoomType.structure(),trapdoorRoomType.biome(),roomRegionId,trapdoorRoomType.chunksSize());
    }
}
